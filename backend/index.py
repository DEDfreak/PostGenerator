...
# Stage 2: Post Generation Template (Original)
generation_template = """
...
def analyze_topic():
    ...
    try:
        ...
        analysis_prompt = f"""
        Analyze this LinkedIn post topic: "{topic}"
        
        Based on this topic, suggest 3-5 specific input fields that would help create more personalized and engaging LinkedIn posts.
        
        Consider what additional details would make the post more authentic and specific.
        
        Return your response as a valid JSON array with this exact format:
        [
            {{"field": "field_name", "label": "Display Label", "description": "Helper text for user", "type": "text"}},
            {{"field": "field_name2", "label": "Display Label 2", "description": "Helper text", "type": "text"}}
        ]
        
        Field types can be: "text", "textarea", "number", "select"
...
@app.route('/api/generate-enhanced', methods=['POST', 'OPTIONS'])
